...
public:
	...
	bool loadFont(string filename,float fontsize, bool bAntiAliased=true, bool makeContours=false, float simplifyAmt=0.3, int dpi=0, bool useTexture=true,string scriptTageName="");
	//add sub font ,What was read afterwards is used strongly. (Or set unicode range correctly . )
	bool loadSubFont(string filename,float sizeRate=1.0f, float baseLineRate=0.0f ,int unicodeRangeStart=0x0000,int unicodeRangeEnd=0x0000 ,string scriptTageName="" );
